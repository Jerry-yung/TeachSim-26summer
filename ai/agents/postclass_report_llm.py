...
from setting.llm import report_llm
...
【生成要求（建议优先遵循）】
1) 若教案 JSON 中包含 `teaching_preferences`，请结合其中字段进行定向分析：
   - grade：学生年级
   - class_type：重点班，普通班，平行班
   - primary_goal：知识理解与记忆，技能掌握与方法运用，思维拓展与探究讨论，情感体验与价值建构
   - breakthrough_focus（训练重点）：课堂节奏把控，知识点衔接流畅度，提问质量与互动引导，语言表达与亲和力，时间分配合理性
2) summary 与 priority_improvements 建议体现上述偏好，尽量避免只做通用评价。
3) priority_improvements 建议至少 1-2 条直接对应训练重点（breakthrough_focus）。
4) overall_score 与 dimension_scores 仍需和片段数据整体一致、可解释。
5) 面向教师展示时尽量使用中文表达，避免英文技术字段名直接出现在正文中（例如不要直接写 breakthrough_focus）。
6) 语气以“建议/可优先/可考虑”为主，避免“必须”这类过强措辞。
7) 若本节课未提供 PPT，请使用“本节课未使用PPT辅助讲解”或同义表达，不要写“PPT内容普遍缺失”。
...
class PostclassReportLLM:
    """汇总 LLM2 段落评价 + 教案 JSON，由 report_llm 输出报告。"""

    def run(self, *, lesson_json: Dict[str, Any], segment_evals: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            chat = report_llm.model if hasattr(report_llm, "model") else report_llm
    ...
    def _fallback(self, lesson_json: Dict[str, Any], segment_evals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        focus_text = _prefs_display_text(prefs.get("breakthrough_focus"))
        ...
        goal_text = _prefs_display_text(prefs.get("primary_goal"))
...
def _prefs_display_text(val: Any) -> str:
    if val is None:
        return ""
...
def _focus_default_items(focus_text: str) -> List[str]:
    """根据 breakthrough_focus 文字内容返回对应的改进建议（支持多选合并文案）。"""
    mapping = {
    ...
    if not focus_text:
        return []
    parts = [p.strip() for p in re.split(r"[、,，]", focus_text) if p.strip()]
    if not parts:
        parts = [focus_text]
    out: List[str] = []
    for p in parts:
