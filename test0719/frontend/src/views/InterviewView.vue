      ...
      <Transition name="fade">
        <div v-if="store.isInterviewComplete" class="complete-card animate-fadeInUp">
          ...
          <div class="complete-summary">
            <div v-for="[key, val] in answeredSummaryEntries" :key="key" class="summary-row">
              <span class="summary-key">{{ questionLabel(key) }}</span>
              <span class="summary-val">{{ formatAnswer(val) }}</span>
            </div>
          </div>
          <p v-if="previewPreparing || previewStatus" class="preview-status">
            {{ previewPreparing ? `课件预转换中：${previewStatus || '正在准备…'}` : previewStatus }}
          </p>
          <p v-if="enterError" class="enter-error">{{ enterError }}</p>
          <button class="enter-btn" :disabled="entering || previewPreparing" @click="enterClassroom">
            {{ previewPreparing ? '课件预转换中…' : (entering ? '课堂初始化中…' : '进入课堂') }}
...
import { fetchLessonPptPreview } from '@/api/ai.js'
import { warmupPptPdfFromUrl } from '@/utils/pptPdf.js'
...
const hasPptCourseware = computed(() => {
  if (store.uploadedPPT) return true
...
async function waitPptPreviewReady(lessonId) {
  const id = String(lessonId || '').trim()
  if (!id) return { ok: true, preview_url: '' }
  previewPreparing.value = true
  previewStatus.value = '已启动转换任务'
  const maxAttempts = 45
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const data = await fetchLessonPptPreview(id)
    if (data?.ready && data?.preview_url) {
      previewStatus.value = '转换完成，可进入课堂'
      previewPreparing.value = false
      return { ok: true, preview_url: String(data.preview_url || '') }
    }
    const msg = String(data?.message || '课件预览准备中')
    previewStatus.value = `${msg}（${attempt}/${maxAttempts}）`
    await sleep(2000)
  }
  previewPreparing.value = false
  previewStatus.value = '课件预转换超时，请稍后重试'
  return { ok: false, preview_url: '' }
}

async function enterClassroom() {
  if (entering.value || previewPreparing.value) return
  enterError.value = ''
  previewStatus.value = ''
  entering.value = true
  try {
    // 每次「进入课堂」都创建新 session，避免复用缓存 sessionId 触发 restart 产生空记录
    store.sessionId = null
    const res = await store.bootstrapClassroomSession(String(route.params.sessionId || ''))
    if (hasPptCourseware.value) {
      const pr = await waitPptPreviewReady(res?.lesson_id || store.lessonId)
      if (!pr.ok) {
        throw new Error('课件仍在预转换中，请稍后再次进入课堂')
      }
      if (pr.preview_url) {
        previewStatus.value = '正在预加载课件显示…'
        previewPreparing.value = true
        try {
          await warmupPptPdfFromUrl(pr.preview_url)
          previewStatus.value = '转换完成，可进入课堂'
        } catch (e) {
          console.warn('[Interview] PDF 预热失败（课堂内将重试）', e?.message || e)
        } finally {
          previewPreparing.value = false
        }
      }
    ...
    router.push({
      path: `/classroom/${res.session_id}`,
      state: { fromInterview: true },
    })
  } catch (err) {
...
/** 年级/学科常来自描述推断或预设，不在 interviewQuestions 里，固定排在摘要最前 */
const SUMMARY_LEADING_KEYS = ['grade', 'subject']

/** 按当前问卷顺序展示，避免预设合并后 Object 键序导致错位；年级、学科始终置顶 */
const answeredSummaryEntries = computed(() => {
  const answers = store.interviewAnswers || {}
  const orderedIds = store.interviewQuestions.map((q) => q.id)
  const seen = new Set()
  const rows = []
  for (const id of SUMMARY_LEADING_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(answers, id)) continue
    const val = answers[id]
    if (!hasSummaryAnswer(val)) continue
    rows.push([id, val])
    seen.add(id)
  }
...
/** 课前描述推断字段不在当前 interviewQuestions 中时，避免摘要显示英文 id */
const SUMMARY_KEY_LABELS = {
  ...
  lesson_goal: '教学目标',
  atmosphere: '课堂氛围',
  discipline_simulation_level: '纪律模拟强度',
}

function questionLabel(id) {
  const q = store.interviewQuestions.find((item) => item.id === id)
  if (q?.label) return q.label
  return SUMMARY_KEY_LABELS[id] ?? id
}
